...
function buildTrivia() {
  const config = get("moduli.trivia", {});

  if (config.attivo === false) return;

  const domande = Array.isArray(config.domande)
    ? config.domande
    : [];
...
const triviaBox = createElement(
  "div",
  "trivia-box"
);

content.appendChild(triviaBox);
card.appendChild(content);
  ...
  function mostraIntroduzione() {
    ...
    if (config.richiediNome !== false) {
      ...
      const button = createElement(
        "button",
        "story-button trivia-start-button",
        config.testoPulsanteInizia ||
          "Inizia il Trivia"
      );
      ...
      button.addEventListener("click", () => {
        nomeInvitato = input.value.trim();

        if (!nomeInvitato) {
          input.classList.add("trivia-input-error");
          input.focus();
          return;
        }
  ...
  function mostraDomanda() {
    ...
    const domanda = createElement(
      "h3",
      "trivia-question",
      domandaConfig.domanda
    );
    ...
    const risposte =
      Array.isArray(domandaConfig.risposte)
        ? domandaConfig.risposte
        : [];
    ...
    triviaBox.appendChild(progress);
    triviaBox.appendChild(domanda);
    triviaBox.appendChild(answers);
  }
  ...
  function mostraRisultato() {
    ...
    const risultatoConfig =
      trovaMessaggioRisultato();
    ...
    const text = createElement(
      "p",
      "trivia-result-text",
      risultatoConfig.testo || ""
    );

    triviaBox.appendChild(score);
    triviaBox.appendChild(title);
    triviaBox.appendChild(text);

    if (
      config.salvaRisultati !== false
    ) {
      salvaRisultatoTrivia();
    }
  }
  ...
  function trovaMessaggioRisultato() {
    const risultati =
      config.risultati || {};
  ...
  /* ========================================================
     ...
     Quando aggiungeremo l'URL del Google Apps Script
     nel config.js, questa funzione invierà
     automaticamente i risultati al Google Sheet.
     ======================================================== */

  function salvaRisultatoTrivia() {
    const resultsConfig =
      CONFIG.triviaRisultati || {};

    const endpoint =
      resultsConfig.endpoint || "";
    ...
    data.append(
      "totale",
      String(domande.length)
    );

   risposteDate.forEach((item, index) => {
...
/* Galleria non utilizzata */
// buildGallery();

buildEventInfo();

/* Itinerario non utilizzato */
// buildItinerary();
