...
function applyConfig() {
  ...
  setLink(
  "btnCalendar",
  `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent("Inizio Mondiale 2026")}&dates=20260611T210000/20260611T230000&details=${encodeURIComponent("Serata Mondiale 2026 - Preparati a tifare insieme!")}&location=${encodeURIComponent(INVITO_CONFIG.indirizzoMaps || "")}`
);
...
function applyImages() {
  ...
  if (deco.immagine) {
    ...
    coverImage.style.left = "25%";

    if (deco.distanzaDaBusta) {
      coverImage.style.top = deco.distanzaDaBusta;
    }

    if (deco.width) {
      coverImage.style.width = deco.width;
    }
 ...
 const inviteInner = document.querySelector(".invite-inner");
  ...
  if (inviteInner && immagini.sfondoInvito) {
    inviteInner.style.background =
      ...
      url("${immagini.sfondoInvito}") center/cover no-repeat`;
  }
